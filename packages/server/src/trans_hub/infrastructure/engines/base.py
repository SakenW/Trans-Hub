...
class BaseTranslationEngine(ABC, Generic[_ConfigType]):
    ...
    async def atranslate_batch(
        ...
        """[公共 API] 异步翻译一批文本。"""
        if not source_lang:
            return [EngineError(error_message=f"引擎 '{self.name()}' 需要提供源语言。", is_retryable=False)] * len(texts)
        
        try:
            return await self._translate(texts, target_lang, source_lang)
        except Exception as e:
            return [EngineError(error_message=f"引擎执行时发生未知异常: {e}", is_retryable=True)] * len(texts)