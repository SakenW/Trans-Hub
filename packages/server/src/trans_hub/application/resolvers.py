...
class TranslationResolver:
    ...
    async def resolve_with_fallback(
        ...
        """
        [修复] 从数据库解析翻译，现在通过 UoW 访问数据。
        """
        async with self._uow_factory() as uow:
            # 1. 精确匹配 (语言 + 变体)
            result = await uow.translations.get_published_translation(
            ...
            # 3. 查询并应用语言回退链
            fallback_order = await uow.misc.get_fallback_order(project_id, target_lang)
            if fallback_order:
                logger.debug(
                    "开始语言回退",
                    content_id=content_id,
