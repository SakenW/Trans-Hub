...
class TranslationResolver:
    ...
    async def resolve_with_fallback(
        ...
        """
        [最终版] 从数据库解析翻译，并按顺序应用变体和语言回退链。

        这是系统“读模型”的核心算法，严格遵循《白皮书》的回退逻辑。
        """
        # 1. 精确匹配 (语言 + 变体)
        ...
        if result:
            logger.debug(
                "解析成功: 精确匹配",
                content_id=content_id, lang=target_lang, variant=variant_key
            )
            return result[1], result[0]

        # 2. 如果请求了非默认变体，则回退到该语言的默认变体
        if variant_key != "-":
            result = await self._handler.get_published_translation(
        ...
        if fallback_order:
            logger.debug(
                "开始语言回退",
                content_id=content_id, from_lang=target_lang, chain=fallback_order
            )
            for fallback_lang in fallback_order:
                ...
                if result:
                    logger.debug(
                        "解析成功: 语言回退命中",
                        content_id=content_id, hit_lang=fallback_lang
                    )
                    return result[1], result[0]
