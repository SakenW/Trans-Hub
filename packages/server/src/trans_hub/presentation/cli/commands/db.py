...
def db_check() -> None:
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            console.print(
                f"✅ 维护库连接成功: [yellow]{make_url(maint_url).render_as_string(hide_password=True)}[/yellow]"
            )
    except Exception as e:
...
def db_rebuild(
    ...
    console.print(
        Panel(
            "[bold red]警告：此操作将永久删除数据库中的所有数据！[/bold red]",
            border_style="red",
        )
    )
    if not yes:
    ...
    try:
        ...
        db_migrate()
        console.print(
            f"[bold green]✅ 数据库 '{app_db_name}' 已成功重建并迁移！[/bold green]"
        )
    except Exception as e:
...
def db_inspect() -> None:
    ...
    try:
        ...
        console.print(
            "[bold red]错误: `_db_inspect_impl` 模块不存在或无法导入。[/bold red]"
        )
        raise typer.Exit(1)
    except Exception as e:
