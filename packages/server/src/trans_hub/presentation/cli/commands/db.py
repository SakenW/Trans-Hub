...
def db_migrate() -> None:
    ...
    try:
        ...
        alembic_cfg_path = (
            project_root / "packages/server/alembic.ini"
        )  # 假设 alembic.ini 在 server 包下
        ...
        if not alembic_cfg_path.is_file():
            ...
            if not alembic_cfg_path.is_file():
                raise FileNotFoundError(
                    "Alembic 配置文件 'alembic.ini' 未在预设路径找到。"
                )
        ...
        # Alembic 需要同步驱动
        sync_db_url = config.database_url.replace("+aiosqlite", "").replace(
            "+asyncpg", ""
        )
