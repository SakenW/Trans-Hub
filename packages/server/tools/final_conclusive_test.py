...
def run_final_test():
    ...
    if not db_url_str:
        console.print(
            "❌ [失败] 在 .env 文件或环境变量中找不到 TH_MAINTENANCE_DATABASE_URL。"
        )
        return
    ...
    try:
        ...
        console.print(
            f"  - 将用于连接的同步 URL: [cyan]{sync_url.replace(make_url(db_url_str).password or '***', '***')}[/cyan]"
        )
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            console.print(
                f"[bold green]✅✅✅ [终极成功!] 连接成功！查询结果: {result}[/bold green]"
            )
            console.print(
        ...
        console.print(
            "[bold red]❌❌❌ [终极失败!] 连接失败，成功复现了静默崩溃！[/bold red]"
        )
        console.print(f"\n[bold]错误详情:[/bold]\n{e}")
        console.print(
        ...
        console.print(
            "建议的下一步是在一个全新的、干净的操作系统或 Docker 容器中重试。"
        )
