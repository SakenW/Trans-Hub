...
def run_test():
    ...
    try:
        ...
        config = load_config_from_env(
            mode="test",
            strict=True,
            dotenv_path=server_root / ".env.test"
        )
        console.print("✅ [bold green]成功加载 .env.test 配置。[/bold green]")
        console.print(f"   - 用户名: [yellow]{config.database_url.split('//')[1].split(':')[0]}[/yellow]")
        console.print(f"   - 维护库: [yellow]{make_url(config.maintenance_database_url).database}[/yellow]")
    except Exception as e:
        console.print(f"❌ [bold red]加载配置失败: {e}[/bold red]")
        return

    # 2. 准备连接信息
    maint_url = make_url(config.maintenance_database_url).set(drivername="postgresql+psycopg2")
    ...
    try:
        with sync_maint_engine.connect() as conn:
            ...
            console.print(f"[cyan]步骤 2: 正在创建临时数据库 ([yellow]{test_db_name}[/yellow])...[/cyan]")
    ...
    try:
        with sync_test_engine.connect() as conn:
            ...
            conn.execute(text("SELECT 1"))
            console.print("✅✅✅ [bold green]终极成功！连接到新数据库成功！[/bold green]")
