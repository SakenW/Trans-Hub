...
from tests.helpers.factories import create_request_data, TEST_SOURCE_LANG
from trans_hub.application.coordinator import Coordinator
from trans_hub_core.interfaces import PersistenceHandler
from trans_hub_core.types import TranslationStatus
from trans_hub.infrastructure.db._schema import ThContent, ThTransHead, ThTransRev, ThResolveCache
...
async def test_upsert_content_is_idempotent(handler: PersistenceHandler, coordinator: Coordinator):
...
async def test_full_revision_lifecycle(handler: PersistenceHandler):
    ...
    assert head_after_publish is not None and head_after_publish.published_rev_id == rev1_id
    ...
    assert head_after_reject is not None and head_after_reject.current_status == TranslationStatus.REJECTED

async def test_cascade_delete_from_content(handler: PersistenceHandler, coordinator: Coordinator):
    ...
    head_id, _ = await handler.get_or_create_translation_head(
        project_id=req["project_id"], content_id=content_id, target_lang="de", variant_key="-"
    )
    ...
    async with coordinator._sessionmaker.begin() as session:
        cache_entry = ThResolveCache(
            project_id=req["project_id"], content_id=content_id, target_lang="de", variant_key="-",
            resolved_rev_id="dummy_rev_id", resolved_payload={"text": "dummy"}, expires_at=datetime.now()
        )
        # 模拟一个 rev 记录以满足外键约束
        dummy_rev = ThTransRev(project_id=req["project_id"], id="dummy_rev_id", content_id=content_id, revision_no=-1, status=TranslationStatus.DRAFT, src_payload_json={})
        session.add_all([dummy_rev, cache_entry])

    # 验证初始状态
    async with coordinator._sessionmaker() as session:
        assert (await session.get(ThContent, content_id)) is not None
        assert (await session.execute(select(ThTransHead).where(ThTransHead.id == head_id))).scalar_one_or_none() is not None
        assert (await session.execute(select(ThResolveCache).where(ThResolveCache.content_id == content_id))).scalar_one_or_none() is not None
    ...
    async with coordinator._sessionmaker() as session:
        ...
        assert (await session.execute(select(ThTransHead).where(ThTransHead.id == head_id))).scalar_one_or_none() is None
        assert (await session.execute(select(ThResolveCache).where(ThResolveCache.content_id == content_id))).scalar_one_or_none() is None