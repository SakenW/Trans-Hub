...
EXISTING_TM_DATA = [
    # 技术文档翻译对
    ("Click the Save button to save your changes.", "zh-CN", "点击保存按钮以保存您的更改。"),
    ("Please enter a valid email address.", "zh-CN", "请输入有效的电子邮件地址。"),
    ("Your password must be at least 8 characters long.", "zh-CN", "您的密码必须至少包含8个字符。"),
...
async def perform_tm_matching(coordinator, tm_id: str) -> Dict[str, List[Tuple[str, float, str]]]:
    """
    对新内容执行TM匹配。
...
async def display_match_results(match_results: Dict[str, List[Tuple[str, float, str]]]) -> None:
    """
    显示匹配结果。
...
async def generate_tm_statistics(match_results: Dict[str, List[Tuple[str, float, str]]]) -> None:
    """
    生成TM使用统计。
