...
class QualityChecker:
    ...
    async def check_terminology_consistency(
        self, source_text: str, target_text: str, target_lang: str
    ) -> List[QualityIssue]:
        """检查术语一致性。"""
        issues = []
        ...
        for english_term, correct_translation in terms.items():
            if english_term in source_lower:
                ...
                if correct_translation not in target_text:
                    # 查找可能的错误术语
                    wrong_terms = self._find_similar_terms(
    ...
    async def check_length_appropriateness(
        self, source_text: str, target_text: str, namespace: str
    ) -> List[QualityIssue]:
        """检查长度适当性。"""
        issues = []
...
async def run_quality_checks(checker: QualityChecker) -> List[QualityReport]:
    ...
    for sample in TRANSLATION_SAMPLES:
        ...
        if report.issues:
            print(
                f"      ⚠️  发现 {len(report.issues)} 个问题，质量分数: {report.overall_score:.1f}"
            )
        else:
...
async def display_quality_reports(reports: List[QualityReport]) -> None:
    ...
    for i, report in enumerate(reports, 1):
        sample = next(
            s for s in TRANSLATION_SAMPLES if s["content_id"] == report.content_id
        )
...
async def generate_quality_statistics(reports: List[QualityReport]) -> None:
    ...
    print(
        f"   • 中等质量 (70-89分): {medium_quality} ({medium_quality / total_samples:.1%})"
    )
    ...
    for report in reports:
        for issue in report.issues:
            ...
            issue_severity_counts[issue.severity] = (
                issue_severity_counts.get(issue.severity, 0) + 1
            )
