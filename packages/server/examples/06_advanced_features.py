...
async def demonstrate_resource_sharing(coordinator, project_ids: Dict[str, str]) -> None:
    """
    演示资源共享功能。
    ...
    shared_terminology = {
        "technical_terms": {
            "API": {"zh-CN": "应用程序接口", "ja-JP": "アプリケーションプログラミングインターフェース"},
...
async def demonstrate_data_import_export(coordinator) -> None:
    ...
    supported_formats = {
        "import_formats": [
            {"format": "JSON", "description": "结构化JSON数据", "use_case": "API响应、配置文件"},
            {"format": "CSV", "description": "逗号分隔值", "use_case": "表格数据、批量内容"},
            {"format": "XML", "description": "可扩展标记语言", "use_case": "文档结构、配置"},
            {"format": "XLIFF", "description": "XML本地化交换文件格式", "use_case": "CAT工具交换"},
            {"format": "TMX", "description": "翻译记忆交换格式", "use_case": "翻译记忆导入"}
        ],
        "export_formats": [
            {"format": "JSON", "description": "结构化JSON数据", "use_case": "应用集成"},
            {"format": "CSV", "description": "逗号分隔值", "use_case": "数据分析"},
            {"format": "XLIFF", "description": "XML本地化交换文件格式", "use_case": "CAT工具"},
            {"format": "PO", "description": "Gettext可移植对象", "use_case": "软件本地化"},
            {"format": "Properties", "description": "Java属性文件", "use_case": "Java应用"}
        ]
    }
    ...
    print(f"      最后导出: {export_op['records_exported']} 条记录 ({export_op['format']}格式)")
    print(f"      文件大小: {export_op['file_size_mb']:.1f}MB, 耗时: {export_op['processing_time']:.1f}秒")
    
    print_success("数据导入导出演示完成", 
...
async def demonstrate_custom_plugins(coordinator) -> None:
    ...
    print_success("自定义插件演示完成", 
                 ...
                 active_plugins=sum(1 for p in available_plugins.values() if p["status"] == "active"))


async def generate_comprehensive_report(coordinator, project_ids: Dict[str, str]) -> None:
    """
    生成综合报告。
