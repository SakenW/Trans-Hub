...
def downgrade() -> None:
    ...
    if dialect == "postgresql":
        ...
        op.execute("DROP FUNCTION IF EXISTS th.is_bcp47(TEXT) CASCADE;")
        op.execute("DROP FUNCTION IF EXISTS th.variant_normalize() CASCADE;")
        op.execute("DROP FUNCTION IF EXISTS th.set_updated_at() CASCADE;")
        op.execute("DROP FUNCTION IF EXISTS th.extract_text(JSONB) CASCADE;")
        op.execute("DROP FUNCTION IF EXISTS th.forbid_uida_update() CASCADE;")
        op.execute(
