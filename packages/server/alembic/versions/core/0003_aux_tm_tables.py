...
def upgrade() -> None:
    ...
    op.create_table(
        "events",
        sa.Column(
            "id", sa.BigInteger(), sa.Identity(), primary_key=True, nullable=False
        ),
        sa.Column("project_id", sa.Text(), nullable=False),
    ...
    op.execute(
        "ALTER TABLE th.locales_fallbacks ADD CONSTRAINT ck_locales_fallbacks_is_array CHECK (jsonb_typeof(fallback_order)='array');"
    )
    op.execute(r"""
    ...
    op.execute(
        "ALTER TABLE th.tm_units ADD CONSTRAINT ck_tm_units_context_tags_is_array CHECK (context_tags IS NULL OR jsonb_typeof(context_tags)='array');"
    )
