...
*   **职责明确 (Clear Separation of Concerns)**: 各组件职责高度内聚。`PersistenceHandler` 只管理数据库交互，`Engine` 只处理具体翻译 API 的逻辑，`Coordinator` 只负责编排整个工作流。
...
*   `last_seen_at` (TIMESTAMP): 用于**垃圾回收**的时间戳，每次 `request` 时更新。
...
### **5.1 `types.py`**
定义了所有核心 DTOs，是系统内部数据流的“血液”。关键 DTO 包括：`TranslationStatus`, `EngineBatchItemResult` (包含 `EngineSuccess` 和 `EngineError`，后者含 **`is_retryable`** 字段，指示错误是否可重试), `TranslationResult`, `ContentItem`。
...
*   **新增 `transaction()` 方法**：提供了一个上下文管理器，允许调用者（如 `Coordinator`）在一个显式的数据库事务中执行多个操作，确保原子性。
...
*   **API**: 提供了 `request()`, `process_pending_translations()`, 和 `run_garbage_collection()` (**垃圾回收**) 等核心公共方法。
...
一个基于令牌桶算法的、线程安全的速率限制器，用于控制对外部 API 的请求频率。
...
sequenceDiagram
    ...
    Coord->>+Handler: stream_translatable_items('jp', [PENDING, FAILED])
    ...
    loop 针对每个翻译批次
        ...
        loop 批次内部的重试尝试
            ...
            Coord->>+Engine: translate_batch(texts, target_lang) 或 atranslate_batch(...)
        ...
        Coord->>Coord: _convert_to_translation_result() (将引擎结果转为 TranslationResult)
...
*   **业务ID (business_id)**: 上层应用传入的、全局唯一的静态字符串，用于追踪一个业务位置与特定内容（Content）的稳定关联。
...
*   **垃圾回收 (GC)**: 一种维护操作，用于定期清理数据库中过时或不再被引用的数据，确保数据库的健康和效率。
...
*   **即席翻译**: 调用 `request()` 时，将 `business_id` 参数设为 `None`。这表示这段文本没有特定的业务来源或上下文，仅为一次性翻译请求。
