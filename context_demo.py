...
def request_and_process(coordinator: Coordinator, tasks: List[Dict[str, Any]], target_lang: str):
    """辅助函数：登记任务并处理它们。"""
    log.info(f"\n---> 开始登记 {len(tasks)} 个任务到 {target_lang} <---")
    for i, task in enumerate(tasks):
        log.info(f"正在登记任务 {i+1}/{len(tasks)}: '{task['purpose']}'",
                 text=task['text'], context=task['context'],
                 lang=target_lang, business_id=task.get('business_id'))
        coordinator.request(
    ...
    results_generator = coordinator.process_pending_translations(target_lang=target_lang)
    ...
    if results:
        ...
        for result in results:
            ...
            log.info(
                ...
                business_id=result.business_id if result.business_id else "无 business_id", # 正确显示 business_id
                source=source_info
            )
    else:
    ...
    try:
        ...
        if re_requested_results:
            ...
            log.info(
                ...
                status=result.status.name,
                engine=result.engine,
                business_id=result.business_id if result.business_id else "无 business_id",
                source= "从缓存获取" if result.from_cache else "新翻译 (应不发生)"
            )
            log.info(f"注意：'business_id' '{result.business_id}' 在 th_sources 中的 last_seen_at 已被更新。")
        else:
