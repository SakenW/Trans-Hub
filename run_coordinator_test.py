...
def test_rate_limiter():
    ...
    config = TransHubConfig(database_url=f"sqlite:///{DB_FILE}", active_engine="debug", engine_configs=EngineConfigs(debug=DebugEngineConfig()))
    coordinator = Coordinator(config=config, persistence_handler=handler, rate_limiter=rate_limiter)
    try:
        ...
        results = list(coordinator.process_pending_translations(target_lang="jp", batch_size=1, max_retries=0))
...
def test_retry_logic():
    ...
    apply_migrations(DB_FILE)
    handler = DefaultPersistenceHandler(db_path=DB_FILE)
    debug_config_with_failure = DebugEngineConfig(fail_on_text="cherry", fail_is_retryable=True)
    config = TransHubConfig(database_url=f"sqlite:///{DB_FILE}", active_engine="debug", engine_configs=EngineConfigs(debug=debug_config_with_failure))
    ...
    try:
        ...
        results = list(coordinator.process_pending_translations(target_lang="jp", max_retries=1, initial_backoff=0.1))
...
def test_garbage_collection():
    ...
    apply_migrations(DB_FILE)
    handler = DefaultPersistenceHandler(db_path=DB_FILE)
    config = TransHubConfig(database_url=f"sqlite:///{DB_FILE}", active_engine="debug", engine_configs=EngineConfigs())
    coordinator = Coordinator(config=config, persistence_handler=handler)
    try:
...
def test_openai_engine_flow():
    ...
    handler = DefaultPersistenceHandler(db_path=DB_FILE)
    try:
        config = TransHubConfig(database_url=f"sqlite:///{DB_FILE}", active_engine="openai", engine_configs=EngineConfigs(openai=OpenAIEngineConfig()))
...
def main():
    if load_dotenv(): print("✅ .env 文件已加载。")
    else: print("⚠️ 未找到 .env 文件。")
