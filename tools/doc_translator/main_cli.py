...
def start_interactive_mode() -> None:
    ...
    if action.startswith("1") or action.startswith("2"):
        source_lang = questionary.text(
            "请输入源语言:", default=DEFAULT_SOURCE_LANG
        ).ask()
        ...
        default_lang = questionary.text(
            "请输入项目的默认语言:", default=DEFAULT_MAIN_LANG
        ).ask()
        if not default_lang:
            log.error("默认语言不能为空。")
            raise typer.Exit(code=1)
