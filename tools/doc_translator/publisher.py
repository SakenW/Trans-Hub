...
class DocPublisher:
    ...
    def publish_root_files(self) -> None:
        ...
        log.info(
            "开始发布根目录文件...",
            source_dir=str(default_lang_root_files_dir.relative_to(self.project_root)),
        )
        ...
        for path in default_lang_root_files_dir.glob("*.md"):
            ...
            if content_lang2:
                # --- 核心变更：添加 id 和 ontoggle 事件来实现互斥切换 ---
                lang1_id = f"{lang1_code}-details"
                lang2_id = f"{lang2_code}-details"

                # 注意 ontoggle 中的双引号需要转义，或者使用不同的引号
                section1 = (
                    f"<details id='{lang1_id}' open ontoggle=\"if(this.open) document.getElementById('{lang2_id}').open = false;\">\n"
                    f"<summary><strong>{lang1_name}</strong></summary>\n\n"
                    f"{content_lang1}\n"
                    f"</details>"
                )
                section2 = (
                    f"<details id='{lang2_id}' ontoggle=\"if(this.open) document.getElementById('{lang1_id}').open = false;\">\n"
                    f"<summary><strong>{lang2_name}</strong></summary>\n\n"
                    f"{content_lang2}\n"
                    f"</details>"
                )
    ...
    def _build_doc_for_lang(self, lang: LangCode, filename: str) -> Optional[Document]:
        ...
        if not file_path.exists():
            log.warning(
                "在发布时未找到对应的语言文件，跳过", lang=lang, filename=filename
            )
            return None

        doc = Document(
            source_path=file_path, source_lang=lang, target_langs=[], is_root_file=True
        )
        parse_document(doc)
