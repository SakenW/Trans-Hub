...
def do_check_db_status():
    """执行一系列检查来验证数据库环境。"""
    console.print(Panel("[bold cyan]🩺 Trans-Hub 数据库环境健康检查[/bold cyan]", expand=False))
    ...
    try:
        ...
        with engine.connect() as conn:
            table.add_row("维护库连接", "[green]✅ 成功[/green]", f"成功连接到 '{MAINTENANCE_DB_URL}'")
            ...
            if can_create_db:
                table.add_row("创建数据库权限", "[green]✅ 拥有[/green]", "用户有 CREATEDB 权限，可以运行测试。")
            else:
                table.add_row("创建数据库权限", "[bold red]❌ 缺失[/bold red]", "用户缺少 CREATEDB 权限。请运行: ALTER USER your_user CREATEDB;")
                errors += 1
    except Exception as e:
        table.add_row("维护库连接", "[bold red]❌ 失败[/bold red]", f"无法连接到 '{MAINTENANCE_DB_URL}'.\n错误: {e}")
        errors += 1

    db_version, head_version = get_alembic_versions()
    if "无法访问" not in str(db_version):
        table.add_row("应用数据库", "[green]✅ 存在[/green]", f"数据库 '{APP_DB_NAME}' 已存在。")
        if db_version == head_version:
            table.add_row("Schema 版本", "[green]✅ 最新[/green]", f"数据库已是最新版本 ({head_version})。")
        else:
            table.add_row("Schema 版本", "[yellow]⚠️ 过期[/yellow]", f"数据库版本: {db_version}, 最新版本: {head_version}。")
            errors += 1
    else:
        table.add_row("应用数据库", "[bold red]❌ 不存在或无法访问[/bold red]", f"数据库 '{APP_DB_NAME}' 需要被创建。")
        errors += 1

    console.print(table)

    if errors == 0:
        console.print(Panel("[bold green]✅ 您的数据库环境已为运行集成测试准备就绪！[/bold green]", border_style="green"))
    else:
        console.print(Panel("[bold red]❌ 您的数据库环境存在问题，请根据提示修复或使用重建功能。[/bold red]", border_style="red"))
    return errors == 0


def do_rebuild():
    """删除并重新创建应用数据库，然后运行所有迁移。"""
    console.print(f"[bold yellow]警告：此操作将永久删除数据库 '{APP_DB_NAME}' 及其所有数据。[/bold yellow]")
    proceed = questionary.confirm(f"您确定要重建数据库 '{APP_DB_NAME}' 吗?", default=False).ask()
    if not proceed:
    ...
    console.print(f"[bold green]✅ 数据库 '{APP_DB_NAME}' 已成功重建并迁移至最新版本！[/bold green]")


def do_clear():
    """连接到应用数据库并清空所有表。"""
    console.print(f"[bold yellow]警告：此操作将永久删除数据库 '{APP_DB_NAME}' 中的所有数据。[/bold yellow]")
    proceed = questionary.confirm(f"您确定要清空数据库 '{APP_DB_NAME}' 吗?", default=False).ask()
    if not proceed:
        console.print("[red]操作已取消。[/red]")
        return
    ...
    console.print(f"[bold green]✅ 数据库 '{APP_DB_NAME}' 的所有数据已成功清空！[/bold green]")


def main_loop():
    """主交互循环。"""
    console.print(Panel("[bold cyan]欢迎使用 Trans-Hub 数据库医生[/bold cyan]", subtitle="请选择一个操作"))
