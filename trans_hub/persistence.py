...
class DefaultPersistenceHandler(PersistenceHandler):
    ...
    def stream_translatable_items(
        self, lang_code: str, statuses: List[TranslationStatus], batch_size: int, limit: Optional[int] = None
        ...
        for i in range(0, len(translation_ids), batch_size):
            ...
            yield [
                ContentItem(content_id=row['content_id'], value=row['value'], context_hash=row['context_hash'])
    ...
    def save_translations(self, results: List[TranslationResult]) -> None:
        ...
        with self.transaction() as cursor:
            ...
            cursor.execute(f"SELECT id, value FROM th_content WHERE value IN ({placeholders})", list(all_contents))
...
if __name__ == "__main__":
    ...
    try:
        ...
        handler.update_or_create_source(text_content="hello", business_id="ui.login.welcome", context_hash=None)
        handler.update_or_create_source(text_content="world", business_id="ui.home.title", context_hash="c1")
        
        # 模拟 Coordinator 创建翻译任务
        with handler.transaction() as cursor:
            # content_id=1 ('hello') 请求翻译成德语
            cursor.execute("INSERT INTO th_translations (content_id, lang_code, status, engine_version) VALUES (1, 'de', 'PENDING', 'n/a')")
            # content_id=2 ('world') 请求翻译成德语 (带上下文)
            cursor.execute("INSERT INTO th_translations (content_id, lang_code, context_hash, status, engine_version) VALUES (2, 'de', 'c1', 'PENDING', 'n/a')")
        print("数据准备完成。")
        ...
        with handler.transaction() as cursor:
            # 验证 "hello" 的翻译
            cursor.execute("SELECT * FROM th_translations WHERE content_id = 1 AND lang_code = 'de'")
            ...
            # 验证 "world" 的翻译
            cursor.execute("SELECT * FROM th_translations WHERE content_id = 2 AND lang_code = 'de'")
