...
async def _async_request_new(
    coordinator: Coordinator, request_data: dict[str, Any]
) -> None:
    try:
...
def request_new(
    ...
    namespace: Annotated[
        str, typer.Option("--namespace", help="内容的命名空间，如 'ui.buttons.v1'。")
    ],
    keys_json: Annotated[
        str,
        typer.Option("--keys-json", help="定位内容的最小上下文键集 (JSON 字符串)。"),
    ],
    source_payload_json: Annotated[
        str,
        typer.Option("--source-payload-json", help="要翻译的源内容 (JSON 字符串)。"),
    ],
    ...
    source_lang: Annotated[
        str | None,
        typer.Option("--source-lang", "-s", help="源语言代码 (可选，若配置中已提供)。"),
    ] = None,
    variant_key: Annotated[
        str, typer.Option("--variant", help="语言内变体 (可选)。")
    ] = "-",
) -> None:
    ...
    request_data = {
        ...
        "source_lang": source_lang or state.config.source_lang,
        "variant_key": variant_key,
    }

    if not request_data["source_lang"]:
        console.print(
            "[bold red]❌ 必须通过 --source-lang 或 TH_SOURCE_LANG 提供源语言。[/bold red]"
        )
