...
def main(
    ...
    if (
        ...
        and len(ctx.protected_args) >= 1
        and ctx.protected_args[0] == "migrate"
    ):
        # 初始化事件循环
        if _global_state.loop is None:
            _global_state.loop = asyncio.new_event_loop()
            asyncio.set_event_loop(_global_state.loop)

        # 不初始化协调器，因为 db migrate 命令不需要它

        if _global_state.shutdown_event is None:
            _global_state.shutdown_event = asyncio.Event()
    ...
    if _global_state.coordinator is None:
        log.info("初始化协调器...")
        _global_state.coordinator = _global_state.loop.run_until_complete(
    ...
    if ctx.invoked_subcommand != "db" or (
        ...
        ctx.obj = State()
        ctx.obj.coordinator = _global_state.coordinator
        ctx.obj.loop = _global_state.loop
        ctx.obj.shutdown_event = _global_state.shutdown_event
...
def run_worker(
    ...
    # 类型安全的属性访问
    if not hasattr(ctx.obj, "coordinator") or not isinstance(
        ctx.obj.coordinator, Coordinator
    ):
        console.print("[red]错误: 协调器未正确初始化[/red]")
        raise typer.Exit(1)
    coordinator: Coordinator = ctx.obj.coordinator

    if not hasattr(ctx.obj, "loop") or not isinstance(
        ctx.obj.loop, asyncio.AbstractEventLoop
    ):
        console.print("[red]错误: 事件循环未正确初始化[/red]")
        raise typer.Exit(1)
    loop: asyncio.AbstractEventLoop = ctx.obj.loop

    if not hasattr(ctx.obj, "shutdown_event") or not isinstance(
        ctx.obj.shutdown_event, asyncio.Event
    ):
        console.print("[red]错误: 关闭事件未正确初始化[/red]")
        raise typer.Exit(1)
    shutdown_event: asyncio.Event = ctx.obj.shutdown_event
...
def run_app() -> None:
    ...
    try:
        ...
        # 确保协调器已关闭
        if _global_state.coordinator:
            log.info("应用程序退出，正在关闭协调器...")
            try:
                if _global_state.loop and _global_state.loop.is_running():
                    # 如果事件循环正在运行，使用 call_soon_threadsafe
                    coordinator = _global_state.coordinator
                    if coordinator:
                        # 显式定义函数以提供更好的类型信息
                        def close_coordinator(c: Coordinator) -> None:
                            asyncio.create_task(c.close())

                        _global_state.loop.call_soon_threadsafe(
                            close_coordinator, coordinator
                        )
                elif _global_state.loop:
                    # 如果事件循环存在但未运行，直接运行关闭协程
                    _global_state.loop.run_until_complete(
                        _global_state.coordinator.close()
                    )
                else:
                    # 如果没有事件循环，创建一个新的来运行关闭协程
                    close_loop = asyncio.new_event_loop()
                    asyncio.set_event_loop(close_loop)
                    close_loop.run_until_complete(_global_state.coordinator.close())
                    close_loop.close()
                log.info("协调器已关闭")
            except Exception as e:
