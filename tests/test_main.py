...
async def coordinator() -> AsyncGenerator[Coordinator, None]:
    ...
    coord = Coordinator(config=config, persistence_handler=handler)
...
async def test_debug_engine_workflow(coordinator: Coordinator):
    ...
    assert results[0].translated_content == f"Translated({text}) to {target_lang}"
...
async def test_translators_engine_workflow(coordinator: Coordinator):
    coordinator.switch_engine("translators")
    ...
    assert len(results) == 1
    assert results[0].status == TranslationStatus.TRANSLATED
    translated_content = results[0].translated_content
    assert translated_content is not None
    print(f"\n[ Translators 引擎翻译结果 ]: {translated_content}\n")
...
async def test_openai_engine_workflow(coordinator: Coordinator):
    ...
    translated_content = results[0].translated_content
    assert translated_content is not None
    print(f"\n[ OpenAI 引擎翻译结果 ]: {translated_content}\n")
...
async def test_garbage_collection_workflow(coordinator: Coordinator):
    ...
    # --- 核心修正：移除 async with 前面多余的 await ---
    ...
    assert gc_stats["deleted_sources"] == 1
